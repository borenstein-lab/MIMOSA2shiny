...
getContributions = function(met_fluxes_final, spec_codes = "", kegg_translate = "", path_key = "", alt_calculate = F){
  if(!identical(spec_codes, "")){
    ...
    true_met_fluxes = met_fluxes_final
...
### Set reaction directionality based on fluxes
set_correct_rxn_dir = function(network, fluxes){
...
#run_all_metabolites, using mimosa2 functions to get CMP scores
#Run_all_metabolites_FBA2 moved to mimsoa2 package
...
############## Generate metabolite categories (alternate)
process_brite_file = function(brite_file){
  brite_dat = fread(brite_file, sep = NULL, skip = 3, header = F)
  brite_dat[,A:=ifelse(substr(V1, 1, 1)=="A", 1, 0)]
  brite_dat[,B:=ifelse(substr(V1, 1, 1)=="B", 1, 0)]
  brite_dat[,C:=ifelse(substr(V1, 1, 1)=="C", 1, 0)]
  brite_dat[,D:=ifelse(substr(V1, 1, 1)=="D", 1, 0)]
  brite_dat[D==1,compound:=sapply(V1, function(x){
    return(strsplit(x, split = "[ ]+")[[1]][2])
  })]
  brite_dat[A==1, Category:=gsub("A<b>", "", gsub("</b>", "", V1, fixed = T), fixed = T)]
  a_inds = brite_dat[,which(A==1)]
  brite_dat[, Category2:=sapply(1:nrow(brite_dat), function(x){
    return(brite_dat[max(a_inds[a_inds <= x]), Category])
  })]
  brite_dat[,Category:=Category2]
  brite_dat[B==1,CategoryB:=gsub(" [Fig]", "", gsub("^B  ", "", V1), fixed = T)]
  b_inds = brite_dat[,which(B==1)]
  brite_dat[, CategoryB2:=sapply(1:nrow(brite_dat), function(x){
    if(brite_dat[x,A] != 1){
      return(brite_dat[max(b_inds[b_inds <= x]), CategoryB])
    } else {
      return(NA)
    }
  })]
  brite_dat[,CategoryB:=CategoryB2]
  brite_dat = brite_dat[1:(nrow(brite_dat)-5)]
  return(brite_dat[D==1, list(compound, Category, CategoryB)])
}

get_hmp_dictionary = function(dictionary_file, brite_files, mets_melt){
  ...
  dictionary[,KEGG:=agora_kegg_mets(gsub("[env]", "[e]", medium, fixed = T))]
  ...
  brite_files = list.files("../../KEGGfiles/KEGG_2018/", pattern = "br.*.keg")
  brite_dat = data.table()
  for(j in 1:length(brite_files)){
    foo = process_brite_file(paste0("../../KEGGfiles/KEGG_2018/", brite_files[j]))
    #foo[,briteFile:=brite_files[j]]
    setnames(foo, c("Category", "CategoryB"), paste0(c("Category", "CategoryB"), j))
    if(j != 1){
      brite_dat = merge(brite_dat, foo, by = "compound", all = T)
    } else {
      brite_dat = foo
    }
  }
  dictionary = merge(dictionary, brite_dat, by.x = "KEGG", by.y = "compound", all.x = T)
  dictionary[,Category:=ifelse(!is.na(Category1), Category1, Category2)]
  dictionary[is.na(Category), Category:=Category3]
  dictionary_sub = dictionary[medium %in% mets_melt[,unique(compound)]]
  dictionary_sub[Metabolite %in% c("Co2+", "Cu2+", "magnesium", "Mn2+", "Zinc", "Siroheme", "Sodium", "Chloride", "Fe2+", "Fe3+", "potassium", "calcium", "O2", "Water", "Diphosphate", "H+", "sulfate", "Ammonium", "Hydrogen sulfide", "Hydrogen","Nitrite", "carbon dioxide", "Orthophosphate", "Urea", "Molybdate"), Category:="Metals and inorganic compounds"]
  dictionary_sub[is.na(Category) & (grepl("ose", Metabolite)|grepl("arab", Metabolite)|grepl("saccharide", Metabolite)|grepl("xylan",Metabolite)|grepl("starch", Metabolite)|Metabolite=="pullulan"|grepl("glycogen", Metabolite)|grepl("Dex", Metabolite)|Metabolite=="Amylopectin"), Category:="Carbohydrates"]
  dictionary_sub[grepl("cholate", Metabolite)|grepl("cholic acid", Metabolite), Category:="Steroids"]
  dictionary_sub[is.na(Category) & grepl("Product", Metabolite), Category:="Peptides"]
  dictionary_sub[is.na(Category) & (Metabolite %in% c("L-cysteinylglycine",  "Glycylleucine", "L-cystine")|grepl("Glycyl", Metabolite)|grepl("alanyl", Metabolite)|grepl("protein", Metabolite)|grepl("tryptophan", Metabolite)), Category:="Peptides"]
  dictionary_sub[(grepl("oate$", Metabolite)|Metabolite %in% c("D-Galacturonate", "Fumarate", "(R)-lactate", "glycolate", "2-Dehydro-3-deoxy-D-gluconate", "N-acetylneuraminate", "butyrate")), Category:="Organic acids"]
  dictionary_sub[Category=="Hormones and transmitters", Category:="Peptides"]
  dictionary_sub[Metabolite %in% c("Reduced riboflavin","Dephospho-CoA", "Ubiquinone-8", "Cob(I)alamin", "2-Demethylmenaquinone 8"), Category:="Vitamins and Cofactors"]
  dictionary_sub[is.na(Category) & (grepl("lycerol", Metabolite)|Metabolite=="D-Mannitol"), Category:="Carbohydrates"]
  dictionary_sub[Metabolite %in% c("NMN", "Inosine", "Deoxyinosine"), Category:="Nucleic acids"]
  dictionary_sub[Metabolite %in% c("phenylacetate", "Spermidine", "Indole", "tryptaminium"), Category:="Peptides"]
  dictionary_sub[grepl("xanth",Metabolite, ignore.case = T), Category:="Nucleic acids"]
  dictionary_sub[Category %in% c("Lipids", "ST  Sterol Lipids", "Steroids"), Category:="Steroid metabolites"]
  dictionary_sub[Category=="Peptides", Category:="Peptides and amino acid metabolites"]
  dictionary_sub[is.na(Category), Category:="Other"]
  dictionary_sub[Category=="Vitamins and Cofactors", Category:="Vitamins and cofactors"]
  dictionary_sub[Category=="Nucleic acids", Category:="Nucleotide metabolites"]
  dictionary_sub[,Category:=factor(Category, levels = c("Carbohydrates", "Organic acids", "Steroid metabolites", "Nucleotide metabolites", "Peptides and amino acid metabolites", "Vitamins and cofactors", "Metals and inorganic compounds", "Other"))]
  dictionary_sub2 = unique(dictionary_sub[,list(medium, Metabolite, Category)])
  return(dictionary_sub2)
